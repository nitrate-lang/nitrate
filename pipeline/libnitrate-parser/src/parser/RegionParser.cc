...
bool parser::parse_region(qparse_t &job, qlex_t *rd, Stmt **node) {
  ...
  { /* First token should be the name of the definition */
    ...
    if (tok.is(qName)) {
      ...
      syntax(tok, "Expected region name in region definition");
    }
  }
  ...
  { /* Next token should be an open curly bracket */
    tok = qlex_next(rd);
    if (!tok.is<qPuncLCur>()) {
      syntax(tok, "Expected '{' after region name in region definition");
    }
  }
  ...
  while (true) {
    ...
    if (tok.is<qKFn>()) {
      ...
      if (!parse_function(job, rd, &method) || !method) {
        syntax(tok, "Expected function definition in region definition");
        return false;
      }
      ...
      if (!tok.is<qKFn>()) {
        syntax(
            tok,
            "Expected function definition after 'static' in region definition");
      }
  ...
  RegionDef *sdef =
      RegionDef::get(name, nullptr, fields, methods, static_methods);
