...
//       static const std::unordered_map<CompositeType, std::string_view>
//       kw_name =
//           {
...
//     case QAST_NODE_VAR: {
//       let x = n->as<VarDecl>();
//       switch (x->get_decl_type()) {
//         case VarDeclType::Var:
//           OBJECT_BEGIN("VarDecl");
//           break;
//         case VarDeclType::Const:
//           OBJECT_BEGIN("ConstDecl");
//           break;
//         case VarDeclType::Let:
//           OBJECT_BEGIN("LetDecl");
//           break;
//       }
//       OBJECT_STR(x->get_name());
//       OBJECT_ARRAY(x->get_attributes());
//       OBJECT_SUB(x->get_type());
//       OBJECT_SUB(x->get_value());
//       OBJECT_END();
//       break;
//     }
...
C_EXPORT char *npar_repr(const npar_node_t *node, bool minify, size_t indent,
  ...
  // const npar_node_t *n = static_cast<const npar_node_t *>(node);

  /* Serialize the AST recursively */
  // serialize_recurse(const_cast<Node *>(n), ss, state);

  std::stringstream ss;
  AST_JsonWriter writer(ss);
  const_cast<npar_node_t *>(static_cast<const npar_node_t *>(node))
      ->accept(writer);
