...
NCC_EXPORT std::optional<std::any> ncc::detail::CatchAllAny(const std::function<std::any()> &expr) {
  try {
    return expr();
  } catch (...) {
    return std::nullopt;
...
NCC_EXPORT bool ncc::detail::CatchAllVoid(const std::function<void()> &expr) {
  try {
    expr();
    return true;
  } catch (...) {
