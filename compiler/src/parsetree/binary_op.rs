...
pub struct BinaryExprBuilder<'a> {
    left: Option<Box<Expr<'a>>>,
    operator: Option<BinaryOperator>,
    right: Option<Box<Expr<'a>>>,
}

impl<'a> BinaryExprBuilder<'a> {
    ...
    pub fn build(self) -> BinaryExpr<'a> {
        let left = self
            .left
            .expect("BinaryExprBuilder left expression must be set");
        let operator = self
            .operator
            .expect("BinaryExprBuilder operator must be set");
        let right = self
            .right
            .expect("BinaryExprBuilder right expression must be set");
