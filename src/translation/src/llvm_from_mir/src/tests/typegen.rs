...
//!
//! Types with nested components (arrays, structs, slices, references, …) are
//! interned via TLS storage. All tests therefore construct their `MirType`
//! inside a `using_storage` scope through a closure.

use crate::test_common::Harness;
use crate::ty::{TypegenCtx, gen_fn_ret_ty, gen_ty};
use nitrate_mir::prelude as mir;

/// Lower a `MirType` (constructed by `make_ty` inside the TLS scope) to an LLVM
/// type string using a fresh module.
...
/// Lower a `MirType` to its function return type string (constructed inside TLS).
fn ret_type_string(harness: &Harness, make_ty: impl FnOnce() -> mir::MirType) -> String {
    mir::using_storage(&harness.store, || {
        let ty = make_ty();
        let module = harness.llvm.create_module("t");
        let mut tctx = TypegenCtx {
...
fn reference_maps_to_ptr() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Reference {
            exclusive: false,
            mutable: false,
...
fn pointer_maps_to_ptr() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Pointer {
            exclusive: false,
            mutable: true,
            to: mir::MirType::I32.into(),
        }),
...
fn function_maps_to_ptr() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Function {
...
fn slice_ref_maps_to_fat_pointer() {
    ...
    assert_eq!(
        type_string(&h, || mir::MirType::SliceRef {
            exclusive: false,
            mutable: false,
...
fn slice_ptr_maps_to_fat_pointer() {
    ...
    assert_eq!(
        type_string(&h, || mir::MirType::SlicePtr {
            exclusive: false,
            mutable: false,
            element_type: mir::MirType::I8.into(),
        }),
...
fn tuple_maps_to_struct() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Tuple {
...
fn array_maps_to_llvm_array() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Array {
...
fn struct_maps_to_named_struct_with_fields() {
    let h = Harness::new();
    assert_eq!(
        type_string(&h, || mir::MirType::Struct {
            name: "Point".into(),
...
fn enum_uses_tag_width_for_variant_count() {
    ...
    // <= 256 variants -> i8 tag
    let s8 = type_string(&h, || mir::MirType::Enum {
        name: "E8".into(),
    ...
    assert!(s8.ends_with(", i8 }"), "unexpected enum layout: {s8}");

    // 257..=65536 variants -> i16 tag
    let s16 = type_string(&h, || {
    ...
    assert!(s16.ends_with(", i16 }"), "unexpected enum layout: {s16}");
}
