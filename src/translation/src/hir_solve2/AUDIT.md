...
- **Location**: Line 2 — `#![allow(clippy::result_unit_err)]`
...
- ### 18. `TypeConstraint` has only `Equal` variant

- **Location**: Lines 6-8
- **Severity**: Low (design limitation)
- **Description**: The constraint system can only express type equality. There's no subtyping, assignability, or "coerces-to" constraint. This limits the solver's ability to handle cases like assigning a more specific type to a more general one.
- **Recommendation**: Consider adding constraint variants like `SubtypeOf(TypeId)` or `AssignableTo(TypeId)`.
...
- **Recommendation**: Add comments explaining the gaps or renumber contiguously.
...
- **Recommendation**: Document that field access type resolution is handled outside the solver, or add constraint propagation if needed.

- ### 36. `Solver::visit_method_call` — self borrow always uses `mutable: false, exclusive: false`

- **Location**: Lines 966-971
