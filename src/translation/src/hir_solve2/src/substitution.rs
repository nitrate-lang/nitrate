...
//!
//! A `Substitution` maps generic parameter indices (from `Type::GenericParam { index }`)
//! and inferred type variable ids (from `Type::Inferred { id }`) to concrete `TypeId`s.
//! The `apply` method recursively walks a type tree, replacing parameter references
...
/// A mapping from generic parameter indices and inferred variable ids to concrete types.
...
impl Substitution {
    /// Apply this substitution to a type, replacing all `GenericParam` and `Inferred`
    /// references with their concrete bindings. Types not in the mapping are returned
    /// unchanged.
    pub fn apply(&self, ty: &Type) -> Type {
        match ty {
            ...
            // Leaf types: no generics to substitute.
            Type::Never { .. }
            | Type::Unit { .. }
            | Type::Bool { .. }
            | Type::U8 { .. }
