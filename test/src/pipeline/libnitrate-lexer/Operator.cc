...
TEST_CASE(Operator, Permute, 18, R"(.<<^=)", {OpDot, OpLShift, OpBitXor, OpSet, Token()});
...
TEST_CASE(Operator, Permute, 24, R"(||=^=<<<<)", {OpLogicOrSet, OpBitXorSet, OpLT, OpROTL, Token()});
...
TEST_CASE(Operator, Permute, 33, R"(++-=<<=>)", {OpInc, OpMinusSet, OpLShift, OpArrow, Token()});
...
TEST_CASE(Operator, Permute, 45, R"(>>=<==<<<=)", {OpRShiftSet, OpLT, OpEq, OpROTLSet, Token()});
...
TEST_CASE(Operator, Permute, 48, R"(...>>>=++)", {OpRange, OpDot, OpROTRSet, OpInc, Token()});
TEST_CASE(Operator, Permute, 49, R"(>>=>=<<<<)", {OpRShiftSet, OpGE, OpROTL, OpLT, Token()});
TEST_CASE(Operator, Permute, 50, R"(/==++=)", {OpSlashSet, OpSet, OpPlus, OpPlusSet, Token()});
TEST_CASE(Operator, Permute, 51, R"(>>=<=~>)", {OpRShiftSet, OpLE, OpBitNot, OpGT, Token()});
TEST_CASE(Operator, Permute, 52, R"(||=<<<!=)", {OpLogicOrSet, OpROTL, OpLogicNot, OpSet, Token()});
TEST_CASE(Operator, Permute, 53, R"(>=~>>&)", {OpGE, OpBitNot, OpRShift, OpBitAnd, Token()});
...
TEST_CASE(Operator, Permute, 56, R"(&&=...<)", {OpLogicAndSet, OpRange, OpDot, OpLT, Token()});
...
TEST_CASE(Operator, Permute, 62, R"(|||=-=)", {OpBitOr, OpLogicOr, OpSet, OpMinusSet, Token()});
...
TEST_CASE(Operator, Permute, 74, R"(-=&&=^^)", {OpMinus, OpSet, OpLogicAndSet, OpLogicXor, Token()});
