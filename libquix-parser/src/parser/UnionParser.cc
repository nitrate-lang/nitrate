...
static bool parse_union_field(qparse_t &job, qlex_t *rd, CompositeField **node) {
  ...
  { /*First token is the field name */
    tok = qlex_next(rd);
    if (!tok.is(qName)) {
      syntax(tok, "Expected field name in union definition");
    }
  ...
  { /* Next token should be a colon */
    tok = qlex_next(rd);
    if (!tok.is<qPuncColn>()) {
      syntax(tok, "Expected colon after field name in union definition");
    }
  }

  { /* Next section should be the field type */
    if (!parse_type(job, rd, &type)) {
      syntax(tok, "Expected field type in union definition");
    }
  }
  ...
  { /* Optional default value */
    if (!tok.is<qOpSet>()) {
      syntax(tok, "Expected '=' or ',' after field type in union definition");
    }
    qlex_next(rd);
    ...
    if (!parse_expr(job, rd,
                    ...
                    &value) ||
        !value) {
      syntax(tok, "Expected default value after '=' in union definition");
    }
  }
...
bool parser::parse_union(qparse_t &job, qlex_t *rd, Stmt **node) {
  ...
  { /* First token should be the name of the definition */
    ...
    if (tok.is(qName)) {
      ...
      syntax(tok, "Expected union name in union definition");
    }
  }
  ...
  { /* Next token should be an open curly bracket */
    tok = qlex_next(rd);
    if (!tok.is<qPuncLCur>()) {
      syntax(tok, "Expected '{' after union name in union definition");
    }
  }
  ...
  while (true) {
    { /* Check for the end of the content */
      tok = qlex_peek(rd);
      if (tok.is(qEofF)) {
        syntax(tok, "Unexpected end of file in union definition");
        return false;
      }
      if (tok.is<qPuncRCur>()) {
        qlex_next(rd);
    ...
    if (tok.is<qKFn>()) {
      ...
      if (!parse_function(job, rd, &method) || !method) {
        syntax(tok, "Expected function definition in union definition");
        return false;
      }
      ...
      if (!tok.is<qKFn>()) {
        syntax(tok, "Expected function definition after 'static' in union definition");
      }
      ...
      if (!parse_union_field(job, rd, &field)) {
        syntax(tok, "Expected field definition in union definition");
      }
  ...
  { /* Check for an implementation/trait list */
    if (tok.is<qKWith>()) {
      ...
      { /* The implementation list should be enclosed in square brackets ex: [abc, hello] */
        if (!tok.is<qPuncLBrk>()) {
          syntax(tok, "Expected '[' after 'impl' in union definition");
        }
      }
      ...
      while (true) {
        ...
        if (!tok.is(qName)) {
          syntax(tok, "Expected trait name in union definition");
        }
