...
namespace argh {
  ...
  public:
    ...
    // accessor for a parameter with multiple names, give a list of names, get an std::istream that can be used to
    // convert to a typed value. call .str() on result to get as string returns the first value in the list to be found.
  ...
  inline std::string const& parser::operator[](size_t ind) const {
    if (ind < pos_args_.size()) return pos_args_[ind];
    return empty_;
  }
  ...
  inline string_stream parser::operator()(size_t ind) const {
    if (pos_args_.size() <= ind) return bad_stream();
  ...
  inline std::unordered_map<std::string, std::vector<std::string>> parser::params(
      std::initializer_list<char const* const> init_list) const {
    std::unordered_map<std::string, std::vector<std::string>> result;
    for (auto& name : init_list) {
      for (const auto& param : params(name)) {
        result[name].push_back(param.second);
      }
    }
    return result;
  }
